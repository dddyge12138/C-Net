...
int event_loop_handle_pending_add(struct event_loop *eventLoop, int fd, struct channel *channel) {
    yy_msgx("add channel fd == %d, %s", fd, eventLoop->thread_name);
...
int event_loop_handle_pending_update(struct event_loop *eventLoop, int fd, struct channel *channel) {
    yy_msgx("update channel fd == %d, %s", fd, eventLoop->thread_name);
    struct channel_map *map = eventLoop->channelMap;
...
#ifdef EPOLL_ENABLE
    yy_msgx("set epoll as dispatcher, %s", eventLoop->thread_name);
...
int event_loop_run(struct event_loop *eventLoop) {
    ...
    yy_msgx("event loop run, %s", eventLoop->thread_name);
    ...
    yy_msgx("event loop end, %s", eventLoop->thread_name);
    return 0;
}